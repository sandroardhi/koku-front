...
const fetchKategoriDetail = async () => {
  ...
  try {
    ...
    const { data } = await kategori_repository.show(kategori_id)
...
const organizedProducts = computed(() => {
  ...
  if (Array.isArray(produks.value)) {
    productsArray = produks.value
  } else if (typeof produks.value === 'object') {
    productsArray = Object.values(produks.value)
  }
...
<template>
  ...
  <div class="w-[80%] mx-auto p-4 mb-10">
    ...
    <div v-else>
      <h2 class="w-full h-10 mt-5 flex justify-start items-center text-4xl border-b font-semibold">
        <p v-if="kategori" class="mb-2">
          {{ kategori.nama }}
        </p>
      </h2>
      <div v-for="kantin in organizedProducts" :key="kantin.id">
        ...
        <ul>
          <li v-for="product in produks" :key="product.id">
