...
const fetchOrderProses = async () => {
  isLoading.value = true
  try {
    const { data } = await order_repository.orderProses()
    orders.value = data
  } catch (error) {
    console.log(error)
  } finally {
...
<template>
  <div class="w-full mx-auto py-4 mb-10">
    ...
    <div v-else>
      ...
      <div v-else>
        <h2
          class="w-full h-10 mt-5 flex justify-center items-center text-2xl border-b py-10 font-semibold break-words"
        >
          Belum ada pesanan apapun disini!
        </h2>
      </div>
    </div>
  </div>
